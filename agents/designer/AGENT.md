...
- (`design-system`) Mục 3 chứa khối `<!-- tier:2 role:...,designer story:PROJ -->` của `shared/PRD.md` (**design intent cấp project**: brand, đối tượng người dùng, tông mong muốn, **app tham chiếu nếu là bài clone**) + `shared/system-spec.md` (platform mục tiêu, mức accessibility bắt buộc). Lần chạy thứ 2 (sau khi người chọn): đọc thêm `shared/design/theme-choice.json`.
