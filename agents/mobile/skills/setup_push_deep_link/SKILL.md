...
**Mục tiêu:** Thiết lập push notification (FCM/APNs) và deep linking (universal link/app link) ở tầng hạ tầng — KHÔNG phải nội dung push cụ thể cho 1 story (đó là `dev-be`/phase `mobile-screen` dùng hạ tầng này khi cần).
...
**Output:** Config key + routing deep link đã hoạt động, ghi vào `shared/capabilities/native.json`.
