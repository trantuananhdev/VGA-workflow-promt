...
**Dùng bởi:** `mobile`, phase `client-shell`.
...
**Mục tiêu:** Đảm bảo native shell đáp ứng ngưỡng kỹ thuật bắt buộc hiện hành của Google Play/App Store TRƯỚC khi phase `client-screen` bắt đầu code lên trên.
...
5.5. KÍCH THƯỚC MÀN HÌNH — đối chiếu shell với shared/design/tokens.json ->
   ...
   - Shell phải xử lý inset hệ thống ở tầng theme/window (edge-to-edge + safe area),
     không để từng story tự bù: `client-screen` khai `responsive.safe_area` theo
...
6. Đối chiếu kết quả với shared/capabilities/client.json — mọi permission liệt kê ở đó
...
**Verify (điều kiện của Gate 3 cho phase `client-shell`):** nếu `violations` không rỗng, phase `client-shell` KHÔNG được emit handoff mở khoá `client-screen` — phải sửa shell trước. Lý do cứng: để `client-screen` code hàng loạt story lên trên 1 shell không đạt chuẩn thì lúc phát hiện phải sửa cả shell VÀ mọi story đã build trên đó.
