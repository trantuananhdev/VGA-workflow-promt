...
2. **Binding hợp lệ:** mọi `components[].binds[].field` phải tồn tại thật trong `shared/contracts/api-contracts.json` (slice của story đó). Field lạ = designer bịa tên, `client-screen` sẽ code sai rồi mới phát hiện. Binding nằm **trong từng component** (`binds[]`), **không** phải mảng `data_bindings` phẳng ở gốc — vì field thuộc về component hiển thị nó, và đó chính là điều kiện để kiểm được "component này có xử lý field rỗng chưa" (điều 8, mã `E17`). Tên `data_bindings_summary` ở handoff là chuyện khác: đó là field **message body** cô đặc, không phải cấu trúc file.
3. **Layout JSON parse được** — nó là input máy đọc cho `client-screen`, không phải mô tả cho người.
4. **Handoff body có đủ field cô đặc** theo `kernel/rules/handoff-contracts.md` cạnh `designer-screen → client-screen`: `ui_states_count`, `data_bindings_summary`, `domains_applied`, `responsive_tiers_covered`, `components_used`, có slot quảng cáo hay không.
...
7. **Component registry hợp lệ:** mọi category trong `shared/design/component-registry/<story_id>.json` có `chosen.url` đã xác minh (`verified_url: true`) HOẶC `chosen: null` + `custom_needed: true` kèm `custom_note`. Không entry nào thiếu cả 2 (xem `agents/designer/skills/component_discovery/SKILL.md`) — bịa 1 lib không tồn tại chỉ lộ ra khi `client-screen` cài dependency thất bại, đắt hơn nhiều so với bắt ở đây. **Ngoại lệ `reused_from_core`:** entry có `reused_from_core` khác rỗng thì không cần lặp lại `alternatives_considered`/`popularity_signal` (đã kiểm ở `component-registry.core.json` lúc Gate 7). Mọi entry còn lại (không `reused_from_core`, `custom_needed: false`) phải có thêm `alternatives_considered` (>= 1 phần tử) và `popularity_signal.value` — chọn 1 lib mà không so sánh với ứng viên khác và không có số liệu phổ biến thật thì không được coi là "đã chọn kỹ".

8. **Từng component kiểm riêng, không kiểm ở mức màn hình** — chạy `python kernel/tools/validate.py`, nhóm mã `E13`-`E22` phải **sạch**. Đây là điều kiện duy nhất trong gate này có công cụ chấm, nên theo `ORCHESTRATOR.md` bất biến #5: **phải chạy công cụ**, đọc file này không tính là đã kiểm. Layout phải đúng `kernel/contracts/screen-layout.schema.json`.
...
| `E22` | **Lỗi kích thước màn hình**: khối chứa thiếu `responsive`; hàng ngang không khai `wrap_behavior` (tràn ngang ở 320dp); `columns` thiếu bậc bắt buộc / không đơn điệu / vượt trần ở bậc compact; `degrade_order` trỏ id không phải con thật, hoặc rỗng khi khối có nhiều con; `min_height_dp` khoá chiều cao quanh text (cắt chữ ở cỡ chữ 200%); `pinned` mà không tôn trọng `safe_area`; thiếu/khai thiếu `responsive_declared` |

9. **Phủ đủ bậc kích thước:** `responsive_declared.tiers_covered` phủ đủ `required_tiers` và `orientations` phủ đủ `target_orientations` của `shared/design/tokens.json → responsive_contract`; `font_scale_verified` ≥ `required_font_scale` (`limits.json → responsive`, mặc định **2.0**); `keyboard_avoidance` ≠ `not_applicable` nếu màn có `input`/`select`/`search_field`. Và mọi khối chứa có `responsive` với `min_height_dp: null` nếu nó chứa text.

   Điều này chặn lớp lỗi **duy nhất trong nhóm hiển thị mà trước đây không mã nào bắt được**: layout vỡ ở máy 320dp, chữ bị cắt ở cỡ chữ hệ thống 200%, bàn phím che đúng cái nút Gửi, bar dính đáy nằm dưới gesture bar. Cả 4 đều kiểm được ở tầng dữ liệu, nhưng trước đây cả hợp đồng máy đọc **không có một field nào** về bề rộng/hướng/cỡ chữ — lever duy nhất là string tự do `group`. Hệ quả đúng bằng lớp lỗi mà `on_null`/`text_overflow` sinh ra để chặn: designer khai "xong", gate pass, `client-screen` code theo, và vỡ chỉ lộ ra trên máy thật của người dùng. Bậc/hướng nào là **bắt buộc** thì do project quyết (`system-spec.md` mục `PROJ` → `design-system` → `responsive_contract`), không phải hằng số trong validator: app chỉ làm phone thì ép khai `medium`/`expanded` là đánh thuế mọi story.

Điều 4 quan trọng với tầng network: nếu thiếu `data_bindings_summary`, `client-screen` buộc phải mở lại `api-contracts.json` — mất đúng lợi ích "cô đặc message" mà hợp đồng cạnh thiết kế ra.

Điều 8 là thứ chuyển "bug vặt" từ **phát hiện ở Gate 4 / người dùng thật** sang **phát hiện ở tầng dữ liệu, trước khi `client-screen` sinh 1 dòng code nào**. Trước đây gate này chỉ đếm được ở mức màn hình, nên 1 layout "đủ state, đủ token" vẫn có thể chứa nút bấm dẫn tới state không tồn tại hoặc field null không ai xử lý.
...
Node `<STORY>-designer-screen` → `done`; `RECOMPUTE_READY()` mở khoá `<STORY>-client-screen` nếu `PROJ-client-shell` cũng đã `done`.
