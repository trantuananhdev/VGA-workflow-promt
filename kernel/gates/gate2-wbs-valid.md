...
1. **Mọi `node_id` trong `depends_on` phải TỒN TẠI** trong `nodes` (được phép trỏ sang track khác, vd node `runtime` phụ thuộc node `PROJ-mobile-shell` của track `build`). Trỏ vào node không có = `blocked` vĩnh viễn.
...
5. Với mỗi node, `depends_on` (dịch từ node_id về unit) phải đúng **quy tắc giao tập**:
   ...
   Đây là quan hệ **bằng**, không phải "tập con tuỳ ý" — thiếu 1 dependency đáng lẽ phải có (unit đó CÓ node trong track) là lỗi nghiêm trọng: node sẽ chạy sớm trước khi input sẵn sàng. Cũng không được tự thêm quan hệ chéo (vd `mobile-screen` KHÔNG `depends_on` `dev-be`: 2 track song song, contract-first).
...
7. Node có `unit` mang `only_if` chỉ được tồn tại khi điều kiện đúng — `ads-placement` chỉ cho story có `Monetization: true` trong `shared/PRD.md`.
