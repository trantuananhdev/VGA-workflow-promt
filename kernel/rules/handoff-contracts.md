...
```yaml
artifact_refs:
  - logs/US014-client-screen/flutter-analyze.txt
  - logs/US014-client-screen/flutter-test.txt
```
...
| `cto → design-system` | Platform mục tiêu + min OS (ảnh hưởng quy ước iOS HIG vs Material), **dải kích thước màn hình mục tiêu + có hỗ trợ tablet/landscape không** (nguồn duy nhất để `design-system` điền `responsive_contract`; thiếu = mọi story phải khai cả 4 bậc dù app chỉ làm phone), mức accessibility bắt buộc đạt, ràng buộc kỹ thuật ảnh hưởng style (vd theme tối bắt buộc, font hệ thống) | Body + `PRD.md#PROJ` (**design intent cấp project** do `ba` viết: brand, đối tượng người dùng, app tham chiếu nếu clone) + slice `tech-stack.json` (entry `PROJ` — máy đọc, dùng cho `component_discovery`) |
...
| `cto → client-shell` | **`delivery_target` + `platform_pack` (+ `stack_pack`)** — thứ quyết định agent nạp tri thức nền tảng nào ở bước 0; danh sách permission/quyền cần khai **kèm `story_id` lý do** (least-privilege); ngưỡng nền tảng tối thiểu (min OS / browserslist); cần push/deep-link (mobile) hay SEO/SSR/PWA (web) không | Body + `system-spec.md#<story>` + `tech-stack.json#PROJ` |
| `cto → devops-infra` | **`delivery_targets`** (quyết định kiểu release ở Gate 6) + stack/`build_system` đã chọn, môi trường cần dựng, ràng buộc hạ tầng | Body + `architecture.md#<story>` + `tech-stack.json#PROJ` |
| `cto → ads-setup` | Platform, yêu cầu vùng địa lý (ảnh hưởng GDPR/ATT) | Body |
| `design-system → designer-screen` | `chosen_theme`, **`token_keys`** (danh sách key phẳng theo 5 nhóm `color`/`typography`/`spacing`/`radius`/`elevation` — để designer-screen không phải mở lại `tokens.json` mọi story), `a11y_measured` (số đo contrast thật từng cặp), **`responsive_contract`** (bậc kích thước + hướng + cỡ chữ tối đa mà project cam kết — để `designer-screen` biết phải phủ bậc nào mà không phải mở lại `tokens.json`), `locked_at`, **`core_components_chosen`** (tên component/lib cấp app đã chọn — không phải url/chi tiết, xem `component-registry.core.json`) | Body. Chỉ mở `shared/design/tokens.json` hoặc `component-registry.core.json` khi cần giá trị/chi tiết thật (bình thường chỉ cần TÊN). |
| `designer-screen → client-screen` | `story_id`, `ui_states_count`, **`data_bindings_summary`** (tên field đã dùng — để `client-screen` không phải mở lại `api-contracts.json`), **`domains_applied`** (+ `draft_domains` nếu có), `token_keys_used`, **`responsive_tiers_covered`** (bậc + hướng + `font_scale_verified` mà layout này đã được nghĩ cho — để `client-screen` biết phải test ở bề rộng/cỡ chữ nào), **`components_used`** (tên component đã chọn cho story + cờ `custom_needed` từng cái — để `client-screen` biết cài dependency nào, tự implement cái nào), có slot quảng cáo hay không | Body + **BẮT BUỘC mở `shared/design/screens/<story>.json`** (hợp đồng layout, phải thực hiện từng field — xem `agents/client/skills/implement_screen_contract/SKILL.md`) + `component-registry/<story>.json`. |
| `client-shell → client-screen` | **`platform_pack` đã dùng** (+ `draft_pack: true` nếu phải bootstrap pack mới) — để `client-screen` không phải mở lại `tech-stack.json`; danh sách permission/feature **đã dựng thật** (khớp `capabilities/client.json`); kết quả skill compliance của pack (`violations: []`) | Body. Không cần đọc lại manifest/plist (mobile) hay header/webmanifest (web). |
| `client-screen → qa` | `story_id`, **kết quả** lint/test (dòng tổng kết, không phải nguyên văn) + `artifact_refs` trỏ file log đầy đủ, PR link + CI status, đã test với mock hay backend thật, **`contract_compliance`** (các tỉ số Gate 3 điều 4 đòi: state/component đã dựng, `binds[]` đã xử lý `on_null`, control đã bind `disabled_when`, input có validation) | Body + `PRD.md#<story>` (QA cần acceptance criteria gốc để đối chiếu — đây là ngoại lệ **cố ý**: QA phải đọc bản gốc, không tin bản cô đặc của Dev) |
...
| `client-screen → ads-placement` | `story_id`, screen đã xong, vị trí slot quảng cáo trong layout | Body + `shared/design/screens/<story>.json` (cần toạ độ slot thật) |
...
## Ngoại lệ cố ý thứ hai: `client-screen` luôn mở hợp đồng layout

Cùng dạng ngoại lệ với QA ở trên, khác lý do. Nguyên tắc cô đặc hoạt động được khi bản cô đặc **đủ để hành động**. Với cạnh `designer-screen → client-screen` thì nó **không đủ**, và không thể làm cho đủ: hợp đồng layout mang dữ liệu **theo từng component** (`binds[].on_null`, `text_overflow`, `interaction.disabled_when`, `validation[]`, `emphasis`, `parent`/`order`) — 1 màn 20 component có hàng trăm quyết định như vậy. Cô đặc chúng vào body sẽ vượt `body_max_lines`/`body_max_chars` ngay, và cô đặc **có mất mát** thì đúng bằng bỏ qua field.

Vì vậy body vẫn giữ đúng vai trò cô đặc (đếm nhanh: `ui_states_count`, `components_used`… để `client-screen` biết **quy mô** việc), còn `screens/<story>.json` là **hợp đồng phải thực hiện từng field**. Đây là lý do `client-screen` có skill riêng để tiêu thụ nó (`implement_screen_contract`) và Gate 3 điều 4 kiểm sự tuân thủ bằng **tỉ số**, không bằng lời.
