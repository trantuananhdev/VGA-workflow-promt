...
| `designer-screen → mobile-screen` | `story_id`, `ui_states_count`, **`data_bindings_summary`** (tên field đã dùng — để mobile không phải mở lại `api-contracts.json`), **`domains_applied`** (+ `draft_domains` nếu có), `token_keys_used`, **`responsive_tiers_covered`** (bậc + hướng + `font_scale_verified` mà layout này đã được nghĩ cho — để `mobile-screen` biết phải test ở bề rộng/cỡ chữ nào), **`components_used`** (tên component đã chọn cho story + cờ `custom_needed` từng cái — để `mobile-screen` biết cài dependency nào, tự implement cái nào), có slot quảng cáo hay không | Body + **BẮT BUỘC mở `shared/design/screens/<story>.json`** (hợp đồng layout, phải thực hiện từng field — xem `agents/mobile/skills/implement_screen_contract/SKILL.md`) + `component-registry/<story>.json`. |
