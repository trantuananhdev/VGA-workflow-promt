...
| `design-system → designer-screen` | `chosen_theme`, **`token_keys`** (danh sách key phẳng theo 5 nhóm `color`/`typography`/`spacing`/`radius`/`elevation` — để designer-screen không phải mở lại `tokens.json` mọi story), `a11y_measured` (số đo contrast thật từng cặp), `locked_at`, **`core_components_chosen`** (tên component/lib cấp app đã chọn — không phải url/chi tiết, xem `component-registry.core.json`) | Body. Chỉ mở `shared/design/tokens.json` hoặc `component-registry.core.json` khi cần giá trị/chi tiết thật (bình thường chỉ cần TÊN). |
| `designer-screen → mobile-screen` | `story_id`, `ui_states_count`, **`data_bindings_summary`** (tên field đã dùng — để mobile không phải mở lại `api-contracts.json`), **`domains_applied`** (+ `draft_domains` nếu có), `token_keys_used`, **`components_used`** (tên component đã chọn cho story + cờ `custom_needed` từng cái — để `mobile-screen` biết cài dependency nào, tự implement cái nào), có slot quảng cáo hay không | Body + **BẮT BUỘC mở `shared/design/screens/<story>.json`** (hợp đồng layout, phải thực hiện từng field — xem `agents/mobile/skills/implement_screen_contract/SKILL.md`) + `component-registry/<story>.json`. |
